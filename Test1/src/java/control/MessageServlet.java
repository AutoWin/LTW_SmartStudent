...
//        processRequest(request, response);
        ...
        try {
            java.util.Date sdf1 = new SimpleDateFormat("yyyy-MM-dd").parse(date);
        ...
        if (action.equals("search")) {
            if(flag)    session.setAttribute("keyDateSearch", sqlDate.toString());
            if (flag) {
                try {
            ...
            } else if (!flag) {  // xu ly ngay dang ky khi sqlDate  <= sqlDateKey
